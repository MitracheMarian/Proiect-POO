...
void Student::Acceseaza_tutorial(vector<Tutorial>& tutoriale, int id_accesat, Utilizator current_user, vector<Istoric>& istoric) {
...
void Student::afiseazaTutorialeDupaTopic(vector<Tutorial>& tutoriale, string topic) {
	for (auto& t : tutoriale) {
		if (t.getTopic() == topic) {
			cout << "--------------------------------------------------------------";
			int id = t.getId();
...
void Student::actualizeaza_istoric(Tutorial t, Utilizator curent_user, vector<Istoric>& istoric) {
...
void Student::dateTime(string& date, string& times) {
